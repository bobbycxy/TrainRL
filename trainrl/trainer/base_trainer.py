...
class PPOTrainer:
    def __init__(
        self, model, value_head, optimizer, reward_model,
    ...
    def train(self, input_ids, attention_mask, prompt_mask, actions, rewards, old_log_probs, old_values):
        ...
        for _ in range(self.ppo_epochs):
            ...
            for batch in dataloader:
                ...
                # Forward pass
                logits, _ = self.model(input_ids_b, attn_mask=attn_mask_b)
                log_probs = get_log_probs(logits, input_ids_b)

                # Value predictions
                hidden_states = self.model.generate_hidden_states(input_ids_b, attn_mask=attn_mask_b)
