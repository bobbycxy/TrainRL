...
"""
Proper REINFORCE training script with real math data, response monitoring, and DDP support.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train LLM with REINFORCE on math problems")
    ...
    if rank == 0:
        ...
        logger.info(f"Starting training run in {output_dir}")
    ...
    if rank == 0:
        logger.info("Starting training with trainer.train() method...")
    
    # Custom callback function to monitor training progress
    class TrainingCallback:
        def __init__(self, monitor, monitor_prompts, monitor_every, rank):
    ...
    # Create callback
    callback = TrainingCallback(monitor, monitor_prompts, args.monitor_every, rank)
    ...
    if rank == 0:
        ...
        try:
            ...
            try:
                import torch
                torch.save({
                    'model_state_dict': trainer._get_unwrapped_model().state_dict(),
                    'optimizer_state_dict': trainer.optimizer.state_dict(),
                    'baseline': trainer.baseline,
                    'config': config_dict
                }, model_path / "pytorch_model.bin")
                logger.info(f"Model manually saved to {model_path}")
